...
void AudioRecorder::startRecorder()
{
    ...
    if (m_audioRecorder->state() == QMediaRecorder::StoppedState) {
        ...
        QAudioEncoderSettings settings;
         settings.setCodec("audio/x-raw, layout=(string)interleaved"); // 使用 raw PCM 数据
//        settings.setCodec(codecsVar.at(0).toString());
        ...
        QString container = "audio/x-wav"; // 直接指定 WAV 格式
...
//        qDebug() << "Supported codecs:" << m_audioRecorder->supportedAudioCodecs();
...
//        qDebug()<<"音量等级"<<levels.at(i)<<endl;
    ...
    for (int i = 0; i < levels.count(); ++i) {
        maxAmplitude = qMax(maxAmplitude, levels[i]);
    }

    /* 设置一个阈值，如果振幅低于此值，则认为没有声音 */
    qreal threshold = 0.0008; // 阈值，可以根据实际情况调整
    ...
    if (m_isSilent) {
            silentCounter++;
            if (silentCounter > 20) { // 持续5个buffer都检测不到声音
                stopRecorder();
                emit recordingStopped();  // 录音停止时发射信号
                silentCounter = 0;
                qDebug()<<"停止录音";

            }
        } else {
            silentCounter = 0; // 如果检测到声音，重置计数器
        }

   qDebug()<<silentCounter;
}
